...
wrong_ks = 0
mu_error_list = numeric(m)
score_list = numeric(m)
sigma_error_list = numeric(m)
...
for (i in 1:m) {
  ...
  results = eval_EM(res = EMR_res, mu_list = mu_list, dim = dim, I2_list=I2_list)
  
  if (results[1] != -1) {
    mu_error_list[i] = results[["mu_error"]]
    score_list[i] = results[["score"]]
    sigma_error_list[i] = results[["sigma_error"]]
    
  } else {
    wrong_ks = wrong_ks + 1
    mu_error_list[i] = -1
    score_list[i] = -1
    sigma_error_list[i] = -1
  }
  
}


all_results = data.frame(cbind(mu_error_list, score_list, sigma_error_list))
...
kable(useful_results, col.names = c("mean mu error", "center accuracy score", "mean sigma error"),digits = 4)
...
a.  performs the same as original EM with correct cluster guess, performs better than the original with wrong guesses.
